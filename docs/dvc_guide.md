# DVC 数据版本管理指南

## 简介

本项目使用 DVC (Data Version Control) 来管理大型数据集的版本控制。DVC 允许我们像管理代码一样管理数据，同时将实际的数据文件存储在远程存储（MinIO S3）中，而不是 Git 仓库中。

## 一、DVC 常用指令

### 1.1 拉取数据集

从远程存储下载数据集到本地：

```bash
# 拉取所有数据集
dvc pull

# 拉取特定数据集
dvc pull data/database.dvc
dvc pull data/knowledge_base.dvc
dvc pull data/benchmarks.dvc
dvc pull cache.dvc
```

### 1.2 推送数据集

将本地数据集上传到远程存储：

```bash
# 推送所有数据集
dvc push

# 推送特定数据集
dvc push data/database.dvc
dvc push data/knowledge_base.dvc
dvc push data/benchmarks.dvc
dvc push cache.dvc
```

### 1.3 查看数据集状态

检查数据集的状态（是否需要拉取或推送）：

```bash
# 查看所有数据集状态
dvc status

# 查看特定数据集状态
dvc status data/database.dvc
```

### 1.4 添加新数据集

将新的数据文件或目录添加到 DVC 跟踪：

```bash
# 添加数据文件或目录
dvc add data/new_dataset/

# 提交 DVC 元数据文件到 Git
git add data/new_dataset.dvc data/.gitignore
git commit -m "feat: 添加新数据集"

# 推送数据到远程存储
dvc push
```

### 1.5 查看数据集列表

列出远程存储中的所有数据集：

```bash
# 列出所有 DVC 跟踪的文件
dvc list . --dvc-only -R

# 查看特定目录下的文件
dvc list . data/database --dvc-only
```

### 1.6 查看数据集版本历史

查看数据集的版本变更历史：

```bash
# 查看特定数据集的 Git 历史
git log -- data/database.dvc

# 查看详细的版本信息（包括 MD5 变化）
git log -p -- data/database.dvc
```

### 1.7 切换数据集版本

回退到特定版本的数据集：

```bash
# 1. 切换到特定的 Git 提交
git checkout <commit-hash> data/database.dvc

# 2. 拉取该版本的数据
dvc checkout data/database.dvc

# 3. 如果要保留这个版本，提交变更
git add data/database.dvc
git commit -m "feat: 回退到数据集版本 <commit-hash>"
```

### 1.8 查看远程存储配置

查看当前的远程存储配置：

```bash
# 查看所有远程存储
dvc remote list

# 查看特定远程存储的详细配置
cat .dvc/config
```

## 二、数据集版本记录

| 数据集名称 | 版本标识 | 创建日期 | 数据大小 | 标签 | 说明 |
|-----------|---------|---------|---------|------|------|
| `merged_pretrain.jsonl`<br>(database) | `fd5f22a` | 2025-10-08 | 10.77 GB | 🔵 默认<br>🟢 ICLR2026 | 预训练数据集第一版，包含合并后的预训练语料，用于所有基准实验 |
| `sentence_trex_data.json`<br>(knowledge_base) | `f3ca080` | 2025-10-08 | 446 MB | 🔵 默认<br>🟢 ICLR2026 | T-REx 知识库句子数据第一版，包含结构化知识三元组，用于所有知识注入实验 |
| `eval_data.json`<br>(benchmarks) | `cd7f9bb` | 2025-10-12 | 28 KB | 🔵 默认<br>🟢 ICLR2026 | 基准测试数据集第一版，用于模型评估和性能验证 |
| `knowledge_cache.pt`<br>`cluster_tokens_single_mapping.json`<br>(cache) | `fda7333` | 2025-10-12 | 466 MB | 🔵 默认<br>🟢 ICLR2026 | 缓存数据第一版，包含知识缓存和聚类token映射，用于加速训练 |

**拉取默认版本**:
```bash
git checkout main
dvc pull
```

---

**注意**: 所有数据集文件本身不会提交到 Git，只有 `.dvc` 元数据文件会被 Git 跟踪。默认使用 v1.0-iclr2026 版本的数据集进行实验。
