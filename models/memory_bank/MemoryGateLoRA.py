...
class LoRALinear(nn.Module):
    """
    低秩分解的 Linear 层 (LoRA-style)
    
    公式: output = W_B(W_A(x))
    其中 W_A: [in_features, rank], W_B: [rank, out_features]
...
class ResidualBlockLoRA(nn.Module):
    """
    Transformer-style Residual Block (Pre-Norm FFN) with LoRA
    Structure: x + LoRA(GELU(LoRA(LayerNorm(x))))
    """
    def __init__(self, dim: int, rank: int, expansion_factor: int = 2, dropout: float = 0.1):
        super().__init__()
        self.norm = nn.LayerNorm(dim)
        self.ffn = nn.Sequential(
            LoRALinear(dim, dim * expansion_factor, rank=rank, bias=True),
            nn.GELU(),
            LoRALinear(dim * expansion_factor, dim, rank=rank, bias=True),
            nn.Dropout(dropout)
        )
...
class MemoryGateLoRA(nn.Module):
    """
    低秩分解版本的基于Product Key Memory的记忆选择门控机制
    
    相比原版本：
    - gate_proj 中的所有 Linear 层使用低秩分解（LoRA-style）
    - 只保留一个 ResBlock（移除第二个 ResBlock）
    - 参数量减少约 92.7%，计算量减少约 50%
    - 保持 Product Key Memory 结构不变
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self.gate_rank = cfg.get("gate_rank", 128)  # LoRA rank
        ...
        # 查询投影层：LoRA + 单 ResBlock 结构
        # 1. Input Projection: LoRA(dim, dim)
        # 2. ResBlock: LoRA-based feature extraction (只保留一个)
        # 3. Final LayerNorm
        # 4. Output Projection: LoRA(dim, knowledge_dim)
        self.gate_proj = nn.Sequential(
            LoRALinear(self.dim, self.dim, rank=self.gate_rank, bias=False),  # Input Projection
            ResidualBlockLoRA(self.dim, rank=self.gate_rank, expansion_factor=2, dropout=cfg["dropout"]),  # ResBlock (只保留一个)
            nn.LayerNorm(self.dim),  # Final Norm before output projection
            LoRALinear(self.dim, self.knowledge_dim, rank=self.gate_rank, bias=False)  # Output Projection
        )
