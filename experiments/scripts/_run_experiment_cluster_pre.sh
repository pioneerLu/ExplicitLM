...
################################################################################
# 步骤4: 处理Hydra输出路径重载 - 配置训练输出目录和命令构建
################################################################################
...
################################################################################
# 步骤5: 记录实验元数据 - 生成实验配置和版本信息的JSON记录
################################################################################
log_info "步骤5/6: 记录实验元数据..."

# 调用Hydra输出处理函数，生成最终的训练参数和命令
process_hydra_output_args

# 生成UTC时间戳（ISO 8601格式），用于国际标准化时间记录
TIMESTAMP=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

# 创建实验元数据JSON文件，记录完整的实验配置信息
# 该文件将被后续阶段使用，并作为实验可重现性的重要依据
cat > "$META_FILE" <<EOF
{
  "experiment": {
    "id": "$EXP_ID",
...
################################################################################
# 步骤6: 保存状态供后续阶段使用 - 生成可供source的状态文件
################################################################################
log_info "步骤6/6: 保存状态信息..."

# 创建集群实验状态文件，该文件将被train和post阶段source调用
# 保存所有必要的变量和路径信息，确保各阶段之间数据传递的一致性
cat > "$STATE_FILE" <<EOF
# 集群实验状态文件 - ${EXP_ID}
# 生成时间: $TIMESTAMP
# 用途：供train和post阶段source调用，传递实验状态和配置信息
...
# 提供详细的下一步操作指导，确保用户了解如何继续实验流程
log_info "📋 下一步操作指导："
log_info "1. 将代码和数据同步到计算节点（如需要，取决于集群环境配置）"
log_info "2. 在计算节点运行训练命令："
log_info "   $FINAL_COMMAND"
echo ""

# 提醒用户状态文件的重要性，确保后续阶段能够正确获取配置信息
log_info "📝 重要提醒：状态文件已保存至 $STATE_FILE"
log_info "   该文件包含所有必要的实验配置，供训练和后续阶段使用"
