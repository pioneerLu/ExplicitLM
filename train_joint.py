#!/usr/bin/env python3
"""
阶段3：联合训练 MemoryGate + 知识融合组件

训练目标：
- 加载预训练的 MemoryGate 和 Fusion（可选）
- 联合训练 MemoryGate + GatedMemoryFusion + memory_norm
- 冻结 Backbone
- 使用完整三损失系统
"""
...
def freeze_parameters_for_joint_training(model, accelerator):
    """
    冻结参数：训练 MemoryGate + GatedMemoryFusion + memory_norm
    冻结：backbone
    """
    Logger("🔒 设置参数冻结策略（阶段3：联合训练）", accelerator)
    
    frozen_params = 0
    trainable_params = 0
    
    for name, param in model.named_parameters():
        # 可训练：memory_gate, gated_memory_fusion, memory_norm
...
def load_pretrained_fusion(model, fusion_path, accelerator):
    """
    加载预训练的知识融合组件权重
    """
    ...
    if not os.path.exists(fusion_path):
        Logger(f"警告: Fusion 权重文件不存在，将从头训练: {fusion_path}", accelerator)
        return False
...
def main():
    parser = argparse.ArgumentParser(description="阶段3：联合训练 MemoryGate + Fusion")
    ...
    parser.add_argument("--pretrained_fusion_path", type=str, default=None, help="预训练 Fusion 权重路径（可选）")
    parser.add_argument("--knowledge_num", type=int, default=256*256, help="记忆库条目数（默认65536）")
    ...
    # Loss 配置（阶段3使用完整三损失系统）
    parser.add_argument("--ce_loss_coef", type=float, default=1.0, help="CE Loss 系数")
    parser.add_argument("--similarity_loss_coef", type=float, default=0.1, help="Similarity Loss 系数")
    parser.add_argument("--diversity_loss_coef", type=float, default=0.05, help="Diversity Loss 系数")
    
    # 输出配置
    parser.add_argument("--output_dir", type=str, default="checkpoints/joint", help="输出目录")
    parser.add_argument("--save_interval", type=int, default=500, help="保存间隔（步数）")
    parser.add_argument("--swanlab_project", type=str, default="explicitlm-joint", help="SwanLab 项目名")
    ...
    Logger("阶段3：联合训练 MemoryGate + Fusion", accelerator)
    Logger("=" * 60, accelerator)
    ...
    # 加载预训练的 Fusion（可选）
    if args.pretrained_fusion_path:
        load_pretrained_fusion(model, args.pretrained_fusion_path, accelerator)
    
    # 设置参数冻结（联合训练）
    freeze_parameters_for_joint_training(model, accelerator)
        ...
        if val_loader is not None:
            ...
            if accelerator.is_main_process:
                ...
                if avg_val_loss < best_val_loss:
                    ...
                    best_path = os.path.join(args.output_dir, "explicitlm_joint_best.pth")
                    torch.save(unwrapped_model.state_dict(), best_path)
                    Logger(f"最佳模型已保存: {best_path} (val_loss={best_val_loss:.4f})", accelerator)
        
        # 保存 epoch checkpoint（包含所有组件）
        if accelerator.is_main_process:
            unwrapped_model = accelerator.unwrap_model(model)
            epoch_path = os.path.join(args.output_dir, f"explicitlm_joint_epoch_{epoch}.pth")
